...
- **Retire `HeroPhoneFirst` dead code** — PR #53 removed all `hero: "phone-first"` theme assignments (the 3 emergency demos switched to `form-card`/`booking-card` for online-booking framing). The `HeroPhoneFirst` function in `src/components/home/hero.tsx` is now unreachable but kept in place since removing it cleanly requires pruning `"phone-first"` from the `HeroPattern` union type in `src/lib/themes/types.ts`. Single-file rename when ready: delete the function + remove the union member + drop the `<PreviewCaption>` import if no other pattern uses it.
