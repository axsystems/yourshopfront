...
| `/demos/[slug]` | `src/app/demos/[slug]/page.tsx` | Themed `<ThemedHome isDemoPreview>` — Phase B immersion: hides `<Pricing>` + `<Showcase>` + `<FAQ>` (Your Shopfront-meta content); adds chrome `<DemoBuyGuarantees>` strip before footer; adds fixed-bottom `<MobileStickyCta>` ("From $99 · cancel anytime", single variant across all 30 themes). Sticky `<DemoSwitcher>` at top for hopping between featured 10. The 3 emergency-mode demos (plumbing/electrical/HVAC) use online-booking hero patterns (`form-card`/`booking-card`) and online-only copy — phone-call CTAs were removed in PR #53 since Your Shopfront's purchase + onboarding is fully online. |
